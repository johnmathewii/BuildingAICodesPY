...
def main(X_train, X_test, y_train, y_test):
    ...
    for i, test_item in enumerate(X_test):
        ...
        nearsum = []
        for ni in range(0, k):
            nearsum.append(y_train[nearest[ni]])
            lines.append(np.stack((test_item, X_train[nearest[ni]])))
        y_predict[i] = np.round(np.mean(nearsum))          # this just classifies everything as 0
    print(y_predict)
